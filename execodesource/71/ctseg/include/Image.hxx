...
template <class VoxelType>
Image<VoxelType> Image<VoxelType>::addBorders(const TCoord *preWidth, const TCoord *postWidth, VoxelType value)
{
    TSize newSize[3];
    const TSize *oriSize = this->getSize();
    ...
    Image<VoxelType> res(newSize);

    typename Image<VoxelType>::iterator it;
    typename Image<VoxelType>::iterator end=res.end();

    std::fill(res.begin(), end,value);
    res.copy((*this), preWidth[0],preWidth[1],preWidth[2]);
